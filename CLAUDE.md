...
`content/youtube/` is exempt as a directory (2026-09-23). Every title in a collected channel archive
is the channel's own wording, captured by the collection workflow and never edited here, so the
archived-title rule applies and nothing in it is respelled. The files are JSON, which has nowhere
to put the disable/enable markers, and the workflow that writes them cannot wrap each hit by hand.
The first collected channel after the gate went in carried a video called "My Favourite …" and
turned the one open PR red until this exemption existed.
