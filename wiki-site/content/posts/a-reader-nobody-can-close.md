...
The way around it is to collect the channel's upload list once and publish that as a feed of its own. I have the machinery for it, and the first channel is done. It is christitustechnical (archive) in the sidebar above, under Tech, and its 237 are the videos it published before my date. A collected feed carries the catalog rather than the last fortnight, and there are two ways to keep it to your date. The first is the filter above, set on that feed when you add it: a collected channel delivers its entire catalog on the first fetch, and the filter is what decides which half of it lands unread. The second is to have the feed cut at your date before it is published, so nothing after it ever reaches the reader and that feed needs no filter at all. Mine are cut that way. Those feeds live in a repository of their own, apart from this blog, with its own note on what a listing there does and does not mean: https://github.com/chargingthefuture/rss-feeds. A channel being collected says one person reads it, and nothing else.
...
The reader I host is a convenience on top of that, and it depends on me. The blog's feed address does not.
