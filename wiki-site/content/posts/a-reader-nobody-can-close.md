...
Now it is set once, per feed, and I do nothing. This is my sidebar:
...
Two kinds of source. The category I called Psyop free holds the ones that will never carry placed content — this blog is one — and those have no filter at all. Everything else carries its own date. Open the feed's settings (the gear beside it under Subscription management), find Filter actions and the box under "Mark an article as read…", and put one line in it:
...
The way around it is to collect the channel's upload list once and publish that as a feed of its own. I have the machinery for it, and the first channel is done — the 237 in the sidebar above are its catalog. A collected feed carries the catalog rather than the last fortnight, so the cutoff above does what you asked it to. Set the filter when you add one of these: a collected channel delivers its entire catalog on the first fetch, and the filter is what decides which half of it lands unread.
