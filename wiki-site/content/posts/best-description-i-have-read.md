---
title: "The best description of it I have read"
date: "2026-09-22"
excerpt: "People keep asking what this is and what the methods are. Somebody answered it years ago, better than I would, and my old Quora space pointed at her answer."
category: "Community"
teaser: "What is this and what are the methods? It is the question that arrives most often, and I am not going to answer it, because Sherri Jenkins already did and did it better than I would. Her answer is a list: following on foot and by car, crowding somebody's space, insults murmured so only the target hears, a car outside the house, staged fights, skits built from things only the target should know, theft and vandalism, rumors, photographs, pets harmed, a phone bugged, mail tampered with, and the family pulled in too. The list is the part that matters, because every item on it is nothing taken alone. That is what makes it work and what makes it impossible to report."
topics:
  - community
  - specterati
---

The question that arrives most often, in one form or another, is what this actually is and what the methods are.

I am not going to answer it. Somebody answered it years ago and did it better than I would have, which is why the Quora space I ran pointed at her answer rather than mine. The description on that space carried the line for anybody arriving who did not know: if you do not know what this is, read Sherri Jenkins. I cited her twice more in posts there, in December 2025 and again in February 2026.

## Her answer

Sherri Jenkins (https://www.quora.com/profile/Sherri-Jenkins-12) wrote it in the space Real GangStalking Victims and Evidence. It stands at https://realgangstalkingvictimsandevidence.quora.com/What-are-gang-stalkers-Why-are-they-following-you-and-showing-up-wherever-you-are-11, with 8.1 thousand views and 72 upvotes, one of ten answers to that question.

Her words, not mine:

> Gang stalking is a systematic form of severe harassment performed by multiple people against a target. The target of such harassment is commonly referred to as a/the "targeted individual". Traditionally people think of a "stalker" as one perpetrator, whereas gang stalking involves multiple perpetrators using various techniques to severely bother or make a target conform to so-called rules of society. Common methods used by perpetrators include following the target on foot, by car and public transportation, crowding the target's space in a public place, murmuring insults under the breath so only the target can hear, sitting in the car outside the target's residence, starting "fights" in public with the target, doing "skits" on the street which involve certain information only the target should know but has been found out via surveillance of the target, stealing and vandalizing the target's possessions, creating and spreading false rumors about a target, slandering the target, taking pictures or videotaping the target, harming the target's pets, bugging the target's phone, and mail tampering. The target's family may also fall victim to gang stalking.

![Sherri Jenkins's answer on Quora, captured September 22, 2026, under the question: What are gang stalkers? Why are they following you and showing up wherever you are? The answer reads: Gang stalking is a systematic form of severe harassment performed by multiple people against a target. The target of such harassment is commonly referred to as a/the "targeted individual". Traditionally people think of a "stalker" as one perpetrator, whereas gang stalking involves multiple perpetrators using various techniques to severely bother or make a target conform to so-called rules of society. Common methods used by perpetrators include following the target on foot, by car and public transportation, crowding the target's space in a public place, murmuring insults under the breath so only the target can hear, sitting in the car outside the target's residence, starting "fights" in public with the target, doing "skits" on the street which involve certain information only the target should know but has been found out via surveillance of the target, stealing and vandalizing the target's possessions, creating and spreading false rumors about a target, slandering the target, taking pictures or videotaping the target, harming the target's pets, bugging the target's phone, and mail tampering. The target's family may also fall victim to gang stalking. Below the answer: 8.1K views, 72 upvotes, 3 shares, one of ten answers.](images/sherri-jenkins-answer-2026-09-22.jpg)

She is listed in the Directory, and has been since December 2025: https://app.chargingthefuture.com/apps/directory/profile/18e1474a-a857-4d66-81c9-2c585c923d24

The words she uses are the inherited ones, and I have left them exactly as she wrote them. The vocabulary this blog uses instead is in the [Dictionary](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/Dictionary).

## What she added underneath

She left a second piece in the comments under her own answer, and it carries the vocabulary and the outcomes the first one does not. Her words again:

> Other interchangeable terms used for "gang stalking" include: group stalking, community stalking (wide area supervised local harassment), cause stalking (stalkers use a "cause" for recruitment), mobbing (organized stalking in the workplace) and street theater (harassment skits done in view of the target out in the community). Some victims of organized stalking claim they are also victimized by electronic harassment
>
> Organized Stalking is a form of terrorism used against an individual in a malicious attempt to reduce the quality of a person's life so they will have a nervous break-down, become incarcerated, institutionalized, experience constant mental, emotional, or physical pain, become homeless, and/or commit suicide. This is done using well-orchestrated accusations, lies, rumors, bogus investigations, setups, frame ups, trumped-up charges, intimidation, overt or covert threats, vandalism, thefts, sabotage, torture, humiliation, emotional terror and general harassment. It is a "ganging up" by members of the community who follow an organizer and participate in a systematic "terrorizing" of an individual.
>
> Organized stalking gang members are given targets' names or have the target identified for them. They usually don't know the target before hand and the gang stalkers are tightly networked with stalking groups in other communities as well.
>
> Skeptics of gang stalking point their finger at the target claiming he/ she is delusional or paranoid, thus furthering the goals of the perpetrators. It is readily reported by victims of gang stalking that law enforcement will ignore their pleas for help and actually participate in the gang stalking themselves along with fire fighters, various utility workers and post office employees.

![Sherri Jenkins's comment under her own answer on Quora, captured September 22, 2026. It reads: Other interchangeable terms used for "gang stalking" include: group stalking, community stalking (wide area supervised local harassment), cause stalking (stalkers use a "cause" for recruitment), mobbing (organized stalking in the workplace) and street theater (harassment skits done in view of the target out in the community). Some victims of organized stalking claim they are also victimized by electronic harassment. Organized Stalking is a form of terrorism used against an individual in a malicious attempt to reduce the quality of a person's life so they will have a nervous break-down, become incarcerated, institutionalized, experience constant mental, emotional, or physical pain, become homeless, and/or commit suicide. This is done using well-orchestrated accusations, lies, rumors, bogus investigations, setups, frame ups, trumped-up charges, intimidation, overt or covert threats, vandalism, thefts, sabotage, torture, humiliation, emotional terror and general harassment. It is a "ganging up" by members of the community who follow an organizer and participate in a systematic "terrorizing" of an individual. Organized stalking gang members are given targets\' names or have the target identified for them. They usually don\'t know the target before hand and the gang stalkers are tightly networked with stalking groups in other communities as well. Skeptics of gang stalking point their finger at the target claiming he/she is delusional or paranoid, thus furthering the goals of the perpetrators. It is readily reported by victims of gang stalking that law enforcement will ignore their pleas for help and actually participate in the gang stalking themselves along with fire fighters, various utility workers and post office employees.](images/sherri-jenkins-comment-2026-09-22.jpg)

Four things in that are worth marking.

The outcomes she names are the ones to end up at: a breakdown, jail, a psychiatric ward, the street, suicide. I have [counted the endings I have watched people reach](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/count-the-recruits) and they are the same list. Nobody arrives there by accident.

Members are handed a name and do not know the person, and the groups are networked across places. That is why [who put you on the list stops mattering after a point](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/who-put-you-on-the-list): whoever started it, every group in reach joins, and they run the same schemes in rotation.

Doubt is not neutral. She puts it plainly — a skeptic pointing at the target furthers the aim. That is the same mechanism as [not being able to argue a smear down](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/you-cannot-argue-a-smear-down): the story is built so that denying it reads as proof of it.

And the people she lists at the end — police, fire fighters, utility workers, postal workers — are the reason the answer here is an economy rather than an appeal. Every one of those is somebody a person's week runs through, and the only thing that changes a week is moving it into hands that are not part of it.

## Why the list is the part that matters

Read what she lists and notice the size of each item. Somebody walks behind you. Somebody stands too close in a queue. A car sits outside. A sentence you said in your kitchen turns up in a stranger's mouth on the sidewalk. Something breaks. A rumor arrives ahead of you at a job.

Every one of those is nothing, taken alone. Not a crime, not a threat, not even rude in a way you could describe to a third person without sounding like the problem. That is not an accident of the methods; it is the requirement they are built to meet. It is also why the reasonable next step — tell somebody — costs a survivor more than staying quiet, because the account of it sounds worse than the events.

Two other things in her list are worth pulling out.

The skits built from information only the target should know are the ones that make people think these people are reading minds. [They are not](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/they-are-not-psychic) — she says it in the same sentence, and says where it comes from: surveillance.

And the last line, that a target's family may fall victim too. That is one door into [the scheme I argue against hardest](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/why-they-want-you-living-with-family): make a person destitute until the only place left is back among relatives.

What I would add to her line is the order, because it runs the reverse of how it sounds. The relatives are not reached after somebody arrives. They are recruited first, so the harassment is already running inside the house on the day the person gets there. For somebody living away from family, that work is finished before the homecoming is engineered at all. Mine started when I was five, so there was no arriving: the recruiting ran over the years we lived under one roof.

Both halves of her line are the same approach seen from two sides. The family is worked on before the person being targeted knows anything is happening at all. The ones who go along become part of it. The ones who do not become targets themselves, which is the way a family falls victim. I was an exception on the timing only: I knew before mine were approached.

Though refusing is the wrong shape for what happens, and the difference matters. Refusing suggests somebody is asked a plain question and answers it. What actually happens is a test of whether a person can be moved, and it usually runs without the person knowing a test is underway. Twice, decades apart, somebody who did not know I was a target tried to get me to slander two other people. Both got upset when I would not. Getting one person to slander another is how the dividing is done, and not taking that bait is what declining to traffic actually looks like — as often instinct as decision.

If these people opened with what they want, nobody would join. Steal this. Sleep with that person. Help us ruin somebody. Those asks arrive later, once a reward has already been taken or a person is in far enough that leaving costs more than staying.

So the test selects, and it selects in the direction people do not expect. Somebody who spots the con early is no use to them as a recruit, and the arrangement keeps only one other box. That is a large part of how people end up on the list in the first place.

The sorting does not stop at a family either. The aim is that everybody ends up on one side or the other, a trafficker or a target, and that the traffickers outnumber the targets. That is how every trafficking ring and every psyop has been run.

Somebody who fits neither box is a problem for that arrangement. Two people I knew would not take part, and proved hard to turn into targets. They were killed.

## What a description is not

It is not a remedy, and she was not offering one. [No method I know of stops this](https://chargingthefuture.github.io/chargingthefuture/article/wiki-site/what-can-be-stopped), mine included.

What a description is good for is smaller and real. It tells a person who has been told they are imagining it that the thing has a shape, a name, and other people who recognize it on sight. Every survivor I have met remembers the first accurate description they read.

After that, the thing to do is not to study it further. It is to move what you can out of reach: a ride, a room, an hour of a trade, arranged with somebody who is not part of it. That is what this economy is, and why it is an economy rather than a support group.

## Where to find it in the app

- [Directory](https://app.chargingthefuture.com/apps/directory) — [guide](https://app.chargingthefuture.com/guide#directory)
- [ClickLog](https://app.chargingthefuture.com/apps/click-log) — [guide](https://app.chargingthefuture.com/guide#click-log)
- [SocketRelay](https://app.chargingthefuture.com/apps/socket-relay) — [guide](https://app.chargingthefuture.com/guide#socket-relay)
- [Foundation](https://app.chargingthefuture.com/apps/foundation) — [guide](https://app.chargingthefuture.com/guide#foundation)
- [Commons — the group chat](https://app.chargingthefuture.com) — [guide](https://app.chargingthefuture.com/guide#commons)

To sign up: https://chargingthefuture.com. It is free, everyone is let in one at a time after a check, and you can use one part of it and ignore the rest.
